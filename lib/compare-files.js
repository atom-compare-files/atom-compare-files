...
var resetStatusBar = function () {
  atom.packages.getActivePackage('status-bar').mainModule.statusBar.remove();
};

var setStatusMessage = function (message, type) {
  resetStatusBar();
  atom.packages.getActivePackage('status-bar').mainModule.statusBar
    .appendLeft('<span id="compare-files-status-bar" class="inline-block ' + type + '">' + message + '</span>');
